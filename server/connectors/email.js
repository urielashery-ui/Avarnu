...
<body style="margin:0;background:#F6F8FC;font-family:Arial,sans-serif;color:#16233F;direction:${dir};text-align:${align}">
...
function benefitsHtml(lead, lang) {
  ...
  return `<div style="background:#FFE7DC;color:#16233F;border-radius:16px;padding:16px;margin:20px 0 8px">
    ...
    <div style="background:#fff;border:1px solid #D6DEEC;border-radius:10px;padding:14px;margin-bottom:10px">
      ...
      ${b.url ? `<a href="${esc(b.url)}" style="color:#2A63C9;font-weight:bold">${esc(b.search ? T("ben.searchForm") : T("item.visit", { site: b.site || "" }))}</a>` : ""}
    </div>`).join("");
}

function moversHtml(movers, lang) {
  if (!movers || !movers.length) return "";
  return `<div style="background:#E4EEFF;border-radius:16px;padding:16px;margin:20px 0 8px">
    ...
    ${movers.map((m) => `<p style="margin:4px 0"><strong>${esc(m.name)}</strong> · <a href="tel:${esc(C.digits(m.phone))}" style="color:#2A63C9" dir="ltr">${esc(m.phone)}</a></p>`).join("")}</div>`;
}

// רשימת העדכונים ללקוח — בשפה שבה מילא את הטופס
export function customerChecklistEmail(lead, ref, cfg, movers) {
  ...
  const groups = C.groups(lang).map((g) => {
    ...
    return `<h3 style="font-size:19px;margin:24px 0 8px">${esc(g.title)}</h3>` + its.map((i) => `
      <div style="background:#fff;border:1px solid #D6DEEC;border-radius:10px;padding:14px;margin-bottom:10px">
        ...
        ${i.url ? `<a href="${esc(i.url)}" style="color:#2A63C9;font-weight:bold">${esc(i.search ? T("item.search") : T("item.visit", { site: i.site || "" }))}</a>` : ""}
      </div>`).join("");
  }).join("");
  ...
  return {
    ...
    html: shell(T("res.title"), `<h1 style="font-size:24px;margin:0 0 8px">${esc(T("email.h1", { name: lead.firstName }))}</h1>
      ...
      <p>${esc(T("email.p2"))}</p>${heNote}${moversHtml(movers, lang)}${benefitsHtml(lead, lang)}
...
export function businessNotifyEmail(lead, ref, cfg) {
  ...
  return {
    ...
    html: shell("פנייה חדשה", `<h1 style="font-size:22px">פנייה חדשה: ${esc(ref)}</h1>
      <p>${esc(kind)}. עיר: ${esc(lead.newCity)}. תאריך מעבר: ${esc(C.fmtDate(lead.moveDate))}. ${lead.checklist.length} גופים, ${lead.benefits.length} הנחות לבדוק.</p>
      <p><a href="${esc(cfg.publicUrl)}/admin/leads/${esc(ref)}" style="color:#2A63C9;font-weight:bold">לפרטים במסך הניהול</a></p>
...
export function moverLeadEmail(lead, mover, tokenUrl) {
  ...
  return {
    ...
    html: shell("בקשת הובלה", `<h1 style="font-size:22px;margin:0 0 8px">שלום ${esc(mover.contact || mover.name)}, יש לכם בקשת הובלה חדשה</h1>
      <p><strong>${esc(lead.firstName + " " + lead.lastName)}</strong> · <a href="tel:${esc(C.digits(lead.phone))}" style="color:#2A63C9">${esc(lead.phone)}</a></p>
      ...
      <p style="margin:18px 0"><a href="${esc(wa)}" style="background:#2A63C9;color:#fff;padding:10px 18px;border-radius:999px;text-decoration:none;font-weight:bold">שליחת וואטסאפ ללקוח</a></p>
      <p>אחרי שיצרתם קשר, עדכנו כאן בלחיצה אחת: <a href="${esc(tokenUrl)}" style="color:#2A63C9;font-weight:bold">עדכון מצב הבקשה</a></p>
...
export function reviewRequestEmail(lead, url) {
  ...
  return {
    ...
    html: shell(T("rev.pageT"), `<h1 style="font-size:22px;margin:0 0 8px">${esc(T("rev.h1", { name: lead.firstName }))}</h1>
      <p>${esc(T("rev.p"))}</p>
      <p style="margin:18px 0"><a href="${esc(url)}" style="background:#2A63C9;color:#fff;padding:10px 18px;border-radius:999px;text-decoration:none;font-weight:bold">${esc(T("rev.btn"))}</a></p>`, lang),
